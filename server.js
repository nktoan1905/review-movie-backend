...
app.get("/callback", async function (req, res) {
  try {
    ...
    res.redirect(
      307,
      "https://itss-group5-review-movie.herokuapp.com/login?token=" + token
    );
  } catch (e) {
...
app.get("/me", async function (req, res) {
  try {
    ...
    var db = new DB();
    var my_data = await db.query(
      "SELECT * FROM Sessions NATURAL JOIN Users WHERE session_id=$1;",
      [req.header("token")]
    );
    await db.end();
...
app.post("/movie/create", async function (req, res) {
  const data = req.body;
  try {
    var movie_id = await Movie.createMovie(data);
    res.end(movie_id);
  } catch (e) {
    res.status(e);
  }
